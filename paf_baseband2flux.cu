...
{
  fprintf (stdout,
	   "paf_baseband2flux - To detect baseband data with original channels and average the detected data in time\n"
	   "\n"
	   "Usage: paf_baseband2flux [options]\n"
...
int main(int argc, char *argv[])
{
  ...
  /* Setup log interface */
  sprintf(log_fname, "%s/paf_baseband2flux.log", conf.dir);
  ...
  runtime_log = multilog_open("paf_baseband2flux", 1);
  multilog_add(runtime_log, fp_log);
  multilog(runtime_log, LOG_INFO, "START PAF_BASEBAND2FLUX\n");
  ...
  /* Init process */
  init_baseband2flux(&conf);
  
  /* Do process */
  do_baseband2flux(conf);

  multilog(runtime_log, LOG_INFO, "FINISH PAF_BASEBAND2FLUX\n\n");

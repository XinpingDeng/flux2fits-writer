...
int register_header(conf_t *conf)
{
  ...
  //conf->file_size = conf->bytes_per_second * (conf->length - 8);
  conf->file_size = conf->bytes_per_second * conf->length;
  
  if (ascii_header_set(hdrbuf, "FILE_SIZE", "%.0lf", conf->file_size) < 0)  
    {
...
int acquire_start_time(hdr_t hdr_start, char efname[MSTR_LEN], char utc_start[MSTR_LEN], double *mjd_start, uint64_t *picoseconds)
{
  ...
  *mjd_start = sec/SECDAY + MJD1970;                          // UTC_START = MJD_START
  ...
  //strftime (utc_start, MSTR_LEN, DADA_TIMESTR, gmtime(&now)); // String start time without fraction second
  //fprintf(stdout, "HERE\t%s\n\n\n\n\n", utc_start);
